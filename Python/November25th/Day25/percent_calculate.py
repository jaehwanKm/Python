...
def Calculate():
    ...
    PNL = (total_price * margin_percent)
    total_margin = (total_price + PNL)

    print(f"buy_price = {buy_price :.4f} \nsell_price = {sell_price :.4f} \npercent = {percent :.4f}, \nPNL = {PNL :.4f} \nmargin = {total_margin :.4f}")
    return buy_price, sell_price, percent, PNL, total_margin

buy_price, sell_price, percent, PNL, total_margin = Calculate()     

with open(FILE_PRICE,"a",encoding="utf-8") as f:
    f.write(f"\n{timestamp} \nbuy_price = {buy_price:.4f}, sell_price = {sell_price:.4f}, percent = {percent:.4f}, PNL = {PNL:.4f}, margin = {total_margin:.4f}")
    f.write("\n=============================================================================================")